...
import 'package:project/data/localDataStore/authLocalDataSource.dart';
...
class Api {
  ...
  static Future<dynamic> post(
    ...
    try {
      ...
      if (responseJson['error']) {
        if (errorCode!) {
          throw ApiMessageAndCodeException(
              ...
              errorStatusCode: responseJson["status_code"].toString());
        } else {
      ...
      print(
          "state:: ${apiMessageAndCodeException.errorMessage.toString()} -- ${apiMessageAndCodeException.errorStatusCode.toString()}");
      if (errorCode!) {
        print(
            "instate:: ${apiMessageAndCodeException.errorMessage.toString()} -- ${apiMessageAndCodeException.errorStatusCode.toString()}");
        throw ApiMessageAndCodeException(
  ...
  static Future postApiFile(
    ...
    try {
      ...
      for (var i = 0; i < images.length; i++) {
        ...
        var pic = await http.MultipartFile.fromPath(imagesKey, images[i].path,
  ...
  static Future postApiFileProductRating(Uri url, Map<String, String?> body,
      String? userId, Map<dynamic, File> filelist, String? orderId) async {
    print("Uri is##############$url");
    try {
      var request = http.MultipartRequest('POST', url);
      request.headers.addAll(Api.getHeaders());
...
//api end points
  ...
  static String paypalTransactionWebviewUrl =
      "${baseUrl}paypal_transaction_webview";
  ...
  static String flutterwavePaymentResponseUrl =
      "${baseUrl}flutterwave-payment-response";
  static String getDeliveryChargesUrl = "${baseUrl}get_delivery_charges";
  static String getLiveTrackingDetailsUrl =
      "${baseUrl}get_live_tracking_details";
  ...
  static String createMidtransTransactionUrl =
      "${baseUrl}create_midtrans_transaction";
  static String getMidtransTransactionStatusUrl =
      "${baseUrl}get_midtrans_transaction_status";
  static String midtransWalletTransactionUrl =
      "${baseUrl}midtrans_wallet_transaction";
  ...
  static String searchLocationUrl = "${databaseUrl}search_location";
  static String getLocationDetailsUrl = "${databaseUrl}get_location_details";

  static String apiGetAddressSuggestions =
      "https://maps.googleapis.com/maps/api/place/autocomplete/json?key=${googleAPiKeyAndroid}&input=";
  static String apiGetLatLng =
      "https://maps.googleapis.com/maps/api/geocode/json?key=${googleAPiKeyAndroid}&place_id=";
}
