...
class DesignConfig {
  ...
  static setJpgPath(String name) {
    return "assets/images/jpg/$name.jpg";
  }
    ...
    return (image != "" && image!.isNotEmpty)
        ? FadeInImage(
            placeholder: AssetImage(
              DesignConfig.setPngPath('placeholder'),
            ),
            image: (imageStatus == "1")
                ? AssetImage(
  ...
  static appBar(BuildContext context, double? width, String? text, bottom, {bool? status = false}) {
    return AppBar(
      ...
      leading: InkWell(
          ...
          child: Padding(
              padding: EdgeInsetsDirectional.only(start: width! / 99, top: width /80.00, bottom: width/80.80),
  ...
  static Divider divider() {
    return Divider(color: lightFont.withValues(alpha: 0.85), height: 0.2, thickness: 0.2);
  }
  ...
  Widget bestSellerWidget(double? width, BuildContext context) {
    return Container(
        ...
        decoration: DesignConfig.boxDecorationContainer(yellowColor.withValues(alpha: 0.1), 100),
        child: Row(
