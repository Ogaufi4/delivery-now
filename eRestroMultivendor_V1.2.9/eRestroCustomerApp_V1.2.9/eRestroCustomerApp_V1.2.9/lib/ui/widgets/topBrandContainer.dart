...
class TopBrandContainer extends StatelessWidget {
  ...
  final int index;
  final String? from;
  const TopBrandContainer({Key? key, required this.topRestaurantList, this.width, this.height, required this.index, required this.from})
      : super(key: key);
                ...
                child: ColorFiltered(
                  ...
                  child: ShaderMask(
                    ...
                    child: DesignConfig.imageWidgets(
                        topRestaurantList[index].partnerProfile!, height! / 4.2, from == "home" ? width! / 1.1 : width, "2"),
                  ),
                ),
              ),
                        ...
                        builder: (context, favoriteRestaurantState) {
                          if (favoriteRestaurantState is FavoriteRestaurantsFetchSuccess) {
                            ...
                            bool isRestaurantFavorite =
                                context.read<FavoriteRestaurantsCubit>().isRestaurantFavorite(topRestaurantList[index].partnerId!);
                              ...
                              builder: (context, state) {
                                if (state is UpdateRestaurantFavoriteStatusInProgress) {
                                  return Container(
                                    width: 30.0,
                                    height: 30,
                                    margin: const EdgeInsetsDirectional.only(end: 10.0, top: 10),
                                    child: ClipOval(
                                      ...
                                      onTap: () {
                                        ...
                                        if (isRestaurantFavorite) {
                                          context.read<UpdateRestaurantFavoriteStatusCubit>().unFavoriteRestaurant(
                                              userId: context.read<AuthCubit>().getId(), type: partnersKey, restaurant: topRestaurantList[index]);
                                        } else {
                              ...
                              child: InkWell(
                                onTap: () {
                                  ...
                                  if (context.read<AuthCubit>().state is AuthInitial || context.read<AuthCubit>().state is Unauthenticated) {
                                    Navigator.of(context).pushNamed(Routes.login, arguments: {'from': 'restaurantFavourite'}).then((value) {
                ...
                child: ClipRRect(
                  ...
                  child: BackdropFilter(
                    ...
                    child: Container(
                      ...
                      height: 20,
                      decoration:
                          BoxDecoration(borderRadius: const BorderRadius.all(Radius.circular(5)), color: const Color(0xff000000).withValues(alpha: 0.50)),
                      child: Row(
                        children: [
                          SvgPicture.asset(DesignConfig.setSvgPath("rating"), fit: BoxFit.scaleDown, width: 7.0, height: 12.3),
                          const SizedBox(width: 3.4),
                          Text(
                            double.parse(topRestaurantList[index].partnerRating!).toStringAsFixed(1),
                            textAlign: TextAlign.center,
                            style: const TextStyle(color: white, fontSize: 12, fontWeight: FontWeight.normal),
                          ),
                        ],
                      ),
                    ),
                    ...
                    Column(
                      ...
                      children: [
                        Text(
                          ...
                          textAlign: TextAlign.center,
                          style: const TextStyle(color: white, fontSize: 16, fontWeight: FontWeight.w600, letterSpacing: 0.72),
                        ),
                        ...
                        Row(
                          ...
                          children: [
                            Row(mainAxisSize: MainAxisSize.min, mainAxisAlignment: MainAxisAlignment.start, children: [
                              Text(
                                "${context.read<SystemConfigCubit>().getCurrency()}${topRestaurantList[index].priceForOne!} ${UiUtils.getTranslatedLabel(context, forOneLabel)} | ",
                                textAlign: TextAlign.start,
                                style: const TextStyle(color: white, fontSize: 10, fontWeight: FontWeight.normal, letterSpacing: 0.72),
                              ),
                              SvgPicture.asset(DesignConfig.setSvgPath("time"), fit: BoxFit.scaleDown, width: 7.0, height: 12.3),
                              const SizedBox(width: 5.0),
                              Text(
                                topRestaurantList[index].partnerCookTime!.toString().replaceAll(regex, ''),
                                textAlign: TextAlign.center,
                                style: const TextStyle(color: white, fontSize: 10, fontWeight: FontWeight.normal, letterSpacing: 0.72),
                              ),
                            ]),
