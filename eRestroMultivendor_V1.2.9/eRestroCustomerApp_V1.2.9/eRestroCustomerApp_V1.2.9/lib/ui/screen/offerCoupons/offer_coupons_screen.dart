...
import 'package:project/ui/widgets/simmer/promoCodeSimmer.dart';
...
class OfferCouponsScreenState extends State<OfferCouponsScreen> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      context.read<PromoCodeCubit>().fetchPromoCode(perPage, context.read<GetCartCubit>().cartPartnerId());
    });
  ...
  promoCodeScrollListener() {
    if (promoCodeController.position.maxScrollExtent == promoCodeController.offset) {
      if (context.read<PromoCodeCubit>().hasMoreData()) {
        context.read<PromoCodeCubit>().fetchMorePromoCodeData(perPage, context.read<GetCartCubit>().cartPartnerId());
      }
    }
  }
                                                      ...
                                                      children: [
                                                        ...
                                                        InkWell(
                                                          onTap: () {
                                                            ...
                                                            finalTotalData =
                                                                context.read<GetCartCubit>().getCartModel().overallAmount!.toStringAsFixed(2);
                ...
                ? InkWell(
                    onTap: () {
                      if (addCouponsController.text.trim().isNotEmpty) {
                        ...
                        finalTotalData = context.read<GetCartCubit>().getCartModel().overallAmount!.toStringAsFixed(2);
                        context.read<ValidatePromoCodeCubit>().getValidatePromoCode(promoCodeData, context.read<AuthCubit>().getId(), finalTotalData,
              ...
              body: BlocListener<ValidatePromoCodeCubit, ValidatePromoCodeState>(
                listener: (context, state) {
                  ...
                  if (state is ValidatePromoCodeFetchSuccess) {
                    ...
                    print("promoAmt${promoAmt}--${state.promoCodeValidateModel!.finalDiscount!}");
