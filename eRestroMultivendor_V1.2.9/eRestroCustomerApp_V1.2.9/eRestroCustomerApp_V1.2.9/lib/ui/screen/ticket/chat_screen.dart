...
import 'package:project/data/model/chatModel.dart';
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void _requestDownload(String? url, String? mid, AsyncSnapshot snapshot) async {
    ...
    if (checkpermission) {
      ...
      if (hasExisted) {
        await OpenFilex.open("$_filePath/$fileName");
      ...
      if (response.statusCode == 200) {
        ...
        if (!error) {
          ...
          if (msg != "Ticket Message(s) does not exist")
            UiUtils.setSnackBar(UiUtils.getTranslatedLabel(context, messageLabel), msg!, context, false, type: "2");
        }
        if (mounted) setState(() {});
      }
            ...
            child: Container(
              ...
              child: Row(
                children: <Widget>[
                  ...
                  Expanded(
                    child: Container(
                      decoration: DesignConfig.boxDecorationContainer(black, 100),
                      child: Row(
                        children: [
                          Expanded(
                            child: TextField(
                              ...
                              child: type == "image"
                                  ? Image.network(file,
                                      ...
                                      errorBuilder: (context, error, stackTrace) =>
                                          Image.asset(DesignConfig.setPngPath('placeholder'), height: 150, width: 150))
                                  : Image.asset(
                                      icon,
                                      width: 100,
                                      height: 100,
