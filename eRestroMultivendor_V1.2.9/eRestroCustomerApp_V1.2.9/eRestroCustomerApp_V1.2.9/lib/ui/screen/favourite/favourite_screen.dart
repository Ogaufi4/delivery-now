                      ...
                      if (favouriteProductList[index].availableTime == "1") {
                        if (check == true) {
                          bottomModelSheetShow(
                              context.read<GetCartCubit>().getProductDetailsData(favouriteProductList[index].id!, favouriteProductList[index])[0]);
                        } else {
                          showDialog(
                              context: context,
                              builder: (_) => ProductUnavailableDialog(
                                  startTime: favouriteProductList[index].startTime, endTime: favouriteProductList[index].endTime));
                        }
                      } else {
                        bottomModelSheetShow(
                            context.read<GetCartCubit>().getProductDetailsData(favouriteProductList[index].id!, favouriteProductList[index])[0]);
                    ...
                    PreferredSize(
                      ...
                      child: Container(
                        ...
                        child: TabBar(
                          ...
                          tabs: [
                            Tab(
                              child: Text(UiUtils.getTranslatedLabel(context, productsLabel),
                                  style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500)),
                            ),
                            Tab(
                              child: Text(UiUtils.getTranslatedLabel(context, restaurantsLabel),
                                  style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500)),
                            ),
                          ],
                        ),
