...
import 'package:project/cubit/home/search/filterCubit.dart';
...
class PaymentScreen extends StatefulWidget {
  ...
  const PaymentScreen({Key? key, this.cartModel, this.addNote})
    : super(key: key);
  ...
  static Route<dynamic> route(RouteSettings routeSettings) {
    ...
    return CupertinoPageRoute(
      builder:
          (_) => BlocProvider<FilterCubit>(
            create: (_) => FilterCubit(),
            child: PaymentScreen(
              cartModel: arguments['cartModel'] as CartModel,
              addNote: arguments['addNote'],
            ),
          ),
    );
  }
...
class PaymentScreenState extends State<PaymentScreen> {
      ...
      if (response.statusCode == 200) {
        ...
        if (!error) {
          ...
          for (int i = 0; i < cartModel.data!.length; i++) {
            codAllowedList.add(
              cartModel.data![i].productDetails![0].codAllowed!,
            );
  ...
  Future<void> phonepeCheckSum(String orderId) async {
    try {
      ...
      Response response = await post(
        Uri.parse(Api.phonepeCheckSumUrl),
        body: parameter,
        headers: Api.getHeaders(),
      ).timeout(const Duration(seconds: 50));
      var getdata = json.decode(response.body);
              ...
              if (status == 'SUCCESS') {
                ...
                await Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder:
                        (context) =>
                            ThankYouForOrderScreen(orderId: orderId.toString()),
                  ),
                );
  ...
  Future<void> placeOrder(String? tranId) async {
    try {
      ...
      print(
        "cartMdel:${cartModel.variantId}----${(context.read<GetCartCubit>().getCartModel().variantId)}",
      );
      varientId = cartModel.variantId!.join(",");
      for (int i = 0; i < cartModel.data!.length; i++) {
        quantity =
            quantity != null
                ? "$quantity,${cartModel.data![i].qty!}"
                : cartModel.data![i].qty!;
      }
      ...
      var parameter = {
        userIdKey: context.read<AuthCubit>().getId(),
        mobileKey:
            context.read<AddressCubit>().gerCurrentAddress().mobile == ""
                ? context.read<SystemConfigCubit>().getMobile()
                : context.read<AddressCubit>().gerCurrentAddress().mobile,
        productVariantIdKey: varientId,
        quantityKey: quantity,
        totalKey: overAllAmount.toStringAsFixed(2),
        finalTotalKey: total().toStringAsFixed(2),
        deliveryChargeKey:
            orderTypeIndex.toString() == "0" ? deliveryCharge.toString() : "0",
        ...
        if (!error) {
          ...
          if (paymentMethod ==
            ...
            addTransaction(
              stripePayId,
              orderId,
              tranId == "succeeded" ? placedKey : waitingKey,
              msg,
              true,
            );
          } else if (paymentMethod ==
  ...
  Future<void> paypalPayment(String orderId) async {
    try {
      ...
      Response response = await post(
        Uri.parse(Api.getPaypalLinkUrl),
        body: parameter,
        headers: Api.getHeaders(),
      ).timeout(const Duration(seconds: 50));
      ...
      if (!error) {
        ...
        Navigator.push(
          ...
          CupertinoPageRoute(
            builder:
                (BuildContext context) => PaypalWebView(
                  url: data,
                  from: "order",
                  orderId: orderId,
  ...
  midtrasPayment(
    ...
    try {
      ...
      await post(
            Uri.parse(Api.createMidtransTransactionUrl),
            body: parameter,
            headers: Api.getHeaders(),
          )
          .timeout(const Duration(seconds: 50))
            ...
            (result) {
              ...
              if (!error) {
                ...
                Navigator.push(
                  ...
                  CupertinoPageRoute(
                    builder:
                        (BuildContext context) => MidTrashWebview(
                          url: redirectUrl,
                          from: 'order',
                          orderId: orderID,
                        ),
  ...
  Future<Map<String, dynamic>> updateOrderStatus({
    ...
    var response = await post(
      Uri.parse(Api.updateOrderStatusUrl),
      body: parameter,
      headers: Api.getHeaders(),
    ).timeout(const Duration(seconds: 50));
  ...
  Future<void> deleteOrder(String orderId) async {
    try {
      var parameter = {orderIdKey: orderId};

      Response response = await post(
        Uri.parse(Api.deleteOrderUrl),
        body: parameter,
        headers: Api.getHeaders(),
      ).timeout(const Duration(seconds: 50));
      var getdata = json.decode(response.body);
  ...
  Future<void> addTransaction(
    ...
    try {
      ...
      Response response = await post(
        Uri.parse(Api.addTransactionUrl),
        body: parameter,
        headers: Api.getHeaders(),
      ).timeout(const Duration(seconds: 50));
  ...
  Future<void> flutterWavePayment() async {
    try {
      ...
      Response response = await post(
        Uri.parse(Api.flutterwaveWebviewUrl),
        body: parameter,
        headers: Api.getHeaders(),
      ).timeout(const Duration(seconds: 50));
                ...
                bottomNavigationBar: ButtonContainer(
                  ...
                  onPressed: () {
                    if (paymentMethod == null || paymentMethod!.isEmpty) {
                      ...
                      UiUtils.setSnackBar(
                        UiUtils.getTranslatedLabel(context, paymentLabel),
                        StringsRes.selectPaymentMethod,
                        context,
                        false,
                        type: "2",
                      );
                      ...
                      if (total() <
                        ...
                        if (paymentMethod ==
                            UiUtils.getTranslatedLabel(
                              context,
                              caseOnDeliveryLblLabel,
                            )) {
                          if (codAllowedList.contains("0")) {
                            UiUtils.setSnackBar(
                              UiUtils.getTranslatedLabel(context, paymentLabel),
                              StringsRes.selectAnotherPaymentMethod,
                              context,
                              false,
                              type: "2",
                            );
                    ...
                    return (context.read<AuthCubit>().state is AuthInitial ||
                        ...
                        : BlocConsumer<GetCartCubit, GetCartState>(
                          ...
                          builder: (context, state) {
                            ...
                            final cartList =
                                (state as GetCartSuccess).cartModel;
                            cartModel = cartList;
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          >(
                                            builder: (context, state) {
                                              return Text(
                                                "${context.read<SystemConfigCubit>().getCurrency()}${total().toStringAsFixed(2)}",
                                                style: TextStyle(
                                                  color:
                                                      Theme.of(
                                                        context,
                                                              ...
                                                              setState(() {
                                                                ...
                                                                if (value!) {
                                                                  if (total() <=
                                                                    ...
                                                                    remWalBal =
                                                                        0;
                                                                    walletBalanceUsed = double.parse(
                                                                      context
                                                                          .read<
                                                                            SystemConfigCubit
                                                                          >()
    ...
    return InkWell(
      ...
      child:
          paymentModel.isNotEmpty
              ? RadioItem(paymentModel[index], height: height, width: width)
              : Container(),
    );
  }
}
