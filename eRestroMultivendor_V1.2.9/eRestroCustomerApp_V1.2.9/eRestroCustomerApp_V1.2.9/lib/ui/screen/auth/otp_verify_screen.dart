...
import 'package:project/ui/screen/auth/resendOtpTimerContainer.dart';
...
class _OtpVerifyScreenState extends State<OtpVerifyScreen>
    ...
    if (cartOffList.isNotEmpty) {
      for (int i = 0; i < cartOffList.length; i++) {
        context.read<ManageCartCubit>().manageCartUser(
          ...
          addOnId:
              cartOffList[i]["ADDONID"].isNotEmpty
                  ? cartOffList[i]["ADDONID"]
                  : "",
          addOnQty:
              cartOffList[i]["ADDONQTY"].isNotEmpty
                    ...
                    enableResendOtpButton
                        ? () async {
                          ...
                          if (widget.type == "firebase") {
                            ...
                            context.read<ResendOtpCubit>().resentOtp(
  ...
  login() async {
    await Future.delayed(
      Duration.zero,
      () => context.read<SignInCubit>().signInUser(mobile: widget.mobileNumber),
    );
  }
            ...
            builder:
                (context) => Scaffold(
                  appBar: DesignConfig.appBar(
                    context,
                    width!,
                    UiUtils.getTranslatedLabel(context, otpVerificationLabel),
                    const PreferredSize(
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: PinInputTextField(
                                ...
                                onChanged: (pin) {
                                  debugPrint(
                                    'onChanged execute. pin:$pin${pin.length}-----${controller.text.length}-----${otp.length}',
                                  );
