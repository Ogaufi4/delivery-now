...
import 'package:project/ui/widgets/simmer/bottomCartSimmer.dart';
import 'package:project/ui/widgets/bottomSheetContainer.dart';
import 'package:project/ui/widgets/productItemContainer.dart';
import 'package:project/ui/widgets/simmer/restaurantDetailSimmer.dart';
...
class RestaurantDetailScreen extends StatefulWidget {
  ...
  static Route<dynamic> route(RouteSettings routeSettings) {
    ...
    return CupertinoPageRoute(
      builder:
          (_) => BlocProvider<RestaurantCubit>(
...
class RestaurantDetailScreenState extends State<RestaurantDetailScreen>
  ...
  getCategory() {
    print(
      "rslug->${widget.restaurant!.slug!}--${widget.restaurant!.partnerId!}",
    );
    context.read<RestaurantCategoryCubit>().fetchCategory(
      context,
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
        ...
        scontroller.position.extentAfter < 300) {
  ...
  restaurantClosedWidget() {
    return widget.restaurant!.isRestroOpen == "1"
        ...
        : Positioned.directional(
          ...
          child: Lottie.asset(
            DesignConfig.setLottiePath("closed_restaurents"),
            height: height! / 9.0,
            width: width! / 4.0,
          ),
        );
  ...
  cuisineApi() {
    context.read<RestaurantCuisineCubit>().fetchRestaurantCuisine(
      perPage,
      "",
      widget.restaurant!.slug!,
    );
              ...
              : Scaffold(
                    ...
                    return (context.read<AuthCubit>().state is AuthInitial ||
                        ...
                        : BlocConsumer<GetCartCubit, GetCartState>(
                          ...
                          builder: (context, state) {
                            ...
                            for (
                              int i = 0;
                              i < currentCartModel.data!.length;
                              i++
                            ) {
                                  ...
                                  child: Row(
                                    children: [
                                      Row(
                                        ...
                                        children: [
                                          Text(
                                            "$sum ${UiUtils.getTranslatedLabel(context, itemTagLabel)} | ",
                                            textAlign: TextAlign.center,
                                            maxLines: 1,
                                            style: const TextStyle(
                                              color: white,
                ...
                body: RefreshIndicator(
                  ...
                  child: BlocConsumer<
                    ...
                    RestaurantCuisineState
                  >(
                    bloc: context.read<RestaurantCuisineCubit>(),
                    listener: (context, state) {
                      if (state is RestaurantCuisineSuccess &&
                          state.restaurantCuisineList.isNotEmpty) {
                        print(
                          "categorylen->${state.restaurantCuisineList.length}",
                        );
                        tabController = TabController(
                          length: state.restaurantCuisineList.length,
                          vsync: this,
                        );

                        categoryId =
                            state.restaurantCuisineList[selectedIndex].id;
                        if (selectedIndex == 0) {
                          _firstLoad();
                        }
                      }
                    ...
                    builder: (context, state) {
                      if (state is RestaurantCuisineProgress ||
                          state is RestaurantCuisineInitial) {
                        return RestaurantDetailSimmer(
                          width: width!,
                          height: height!,
                        );
                      }
                                                          ...
                                                          children: [
                                                            Row(
                                                              children: [
                                                                ...
                                                                widget.restaurant!.partnerIndicator ==
                                                                        "1"
                                                                    ? SvgPicture.asset(
                                                                      DesignConfig.setSvgPath(
                                                                        "veg_icon",
  ...
  Widget showPropertiesList(bool isloading) {
    int totallen =
        isloading ? myPropertylist.length + 1 : myPropertylist.length;
        ...
        if (dataItem.variants![0].specialPrice! != "0") {
          off =
              (double.parse(dataItem.variants![0].price!) -
                  ...
                  .toDouble();
          off =
              off * 100 / double.parse(dataItem.variants![0].price!).toDouble();
        }
        return offset > 0 &&
                _isLoadMoreRunning == true &&
                index == (myPropertylist.length - 1)
            ? Center(
              child: CircularProgressIndicator(
                color: Theme.of(context).colorScheme.primary,
              ),
