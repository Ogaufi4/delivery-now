...
import 'package:project/ui/screen/payment/Stripe_Service.dart';
import 'package:project/ui/screen/payment/midtransWebView.dart';
import 'package:project/ui/screen/settings/no_internet_screen.dart';
import 'package:project/ui/screen/payment/payment_radio.dart';
import 'package:project/ui/screen/payment/paypal_webview_screen.dart';
...
class WalletScreenState extends State<WalletScreen> {
  ...
  List<String> transactionType = [
    StringsRes.walletTransaction,
    StringsRes.walletWithdrawTransaction
  ];
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero, () {
      context.read<TransactionCubit>().fetchTransaction(
          perPage, context.read<AuthCubit>().getId(), walletKey);
    });
    withdrawWalletController.addListener(scrollGetWithdrawListener);
    Future.delayed(Duration.zero, () {
      context
          .read<GetWithdrawRequestCubit>()
          .fetchGetWithdrawRequest(perPage, context.read<AuthCubit>().getId());
    });
    context
  ...
  scrollListener() {
    if (controller.position.maxScrollExtent == controller.offset) {
      if (context.read<TransactionCubit>().hasMoreData()) {
        context.read<TransactionCubit>().fetchMoreTransactionData(
            perPage, context.read<AuthCubit>().getId(), walletKey);
      }
    }
  }
  ...
  scrollGetWithdrawListener() {
    if (withdrawWalletController.position.maxScrollExtent ==
        withdrawWalletController.offset) {
      if (context.read<GetWithdrawRequestCubit>().hasMoreData()) {
        context.read<GetWithdrawRequestCubit>().fetchMoreGetWithdrawRequestData(
            perPage, context.read<AuthCubit>().getId());
      }
    }
  }
  ...
  Widget selectTransactionType() {
    return Container(
      decoration: DesignConfig.boxDecorationContainerBorder(
          commentBoxBorderColor, textFieldBackground, 10.0),
      margin: EdgeInsetsDirectional.only(
          top: height! / 99.0, start: width! / 30.0, end: width! / 30.0),
      child: Column(
        children: [
          InkWell(
            onTap: onTap,
  ...
  Future<void> getPaymentMethod() async {
    try {
      ...
      Response response = await post(Uri.parse(Api.getSettingsUrl),
              body: parameter, headers: Api.getHeaders())
      ...
      UiUtils.setSnackBar(UiUtils.getTranslatedLabel(context, paymentLabel),
          StringsRes.somethingMsg, context, false,
          type: "2");
    }
  }
  ...
  Future<void> sendRequest(String txnId, String payMethod,
      {String? OrderID,
      String? status,
      Map<String, dynamic>? jsonDatas,
      String? checksums}) async {
    String orderId =
    ...
    try {
      var parameter = {
        ...
        typeKey: 'credit',
        messageKey:
            (messageController!.text == '' || messageController!.text.isEmpty)
      ...
      if (payMethod.toLowerCase() == "stripe" ||
          payMethod.toLowerCase() == "paypal") {
        parameter[skipVerifyTransactionKey] =
            (payMethod.toLowerCase() == "stripe" ||
                    payMethod.toLowerCase() == "paypal")
                ? "true"
                : "false";
      }
      ...
      Response response = await post(Uri.parse(Api.addTransactionUrl),
              body: parameter, headers: Api.getHeaders())
          .timeout(const Duration(seconds: 50));

      var getdata = json.decode(response.body);
      print(
          "transaction${getdata.toString()}-$parameter${amountController!.text.toString()}");
      if ((payMethod.toLowerCase() == "stripe" ||
          payMethod.toLowerCase() == "paypal")) {
      } else {
        setState(() {
      ...
      if (!error) {
        if (payMethod.toLowerCase() == "phonepe") {
          startPaymentPhonePe(double.parse(amountController!.text.toString()),
              orderId: OrderID!, jsonDatas: jsonDatas, checksums: checksums);
        } else {
          if (mounted) {
            setState(() {
              context
  ...
  Future<void> paypalPayment(String amt) async {
    ...
    try {
      ...
      Response response = await post(Uri.parse(Api.getPaypalLinkUrl),
              body: parameter, headers: Api.getHeaders())
          .timeout(const Duration(seconds: 50));
  ...
  Future<void> flutterWavePayment(String price) async {
    try {
      ...
      Response response = await post(Uri.parse(Api.flutterwaveWebviewUrl),
              body: parameter, headers: Api.getHeaders())
          .timeout(const Duration(seconds: 50));
  ...
  razorpayPayment(double price) async {
    ...
    if (contact != '' && email != '') {
      ...
      if (email == '') {
        UiUtils.setSnackBar(UiUtils.getTranslatedLabel(context, emailLabel),
            StringsRes.emailWarning, context, false,
            type: "2");
      } else if (contact == '') {
        UiUtils.setSnackBar(
            UiUtils.getTranslatedLabel(context, phoneNumberLabel),
            StringsRes.phoneWarning,
            context,
            false,
            type: "2");
      }
  ...
  Future<Map<String, dynamic>> midtransPayment({
    ...
    try {
      ...
      try {
        ...
        await post(Uri.parse(Api.createMidtransTransactionUrl),
                body: parameter, headers: Api.getHeaders())
            .timeout(const Duration(seconds: 50))
            .then(
          (result) {
  ...
  Future<void> phonepeCheckSum(double price) async {
    try {
      String orderID =
          '${context.read<AuthCubit>().getId()}-${DateTime.now().millisecondsSinceEpoch}-${Random().nextInt(900) + 100}';
      var parameter = {
        typeKey: "wallet",
      ...
      Response response = await post(Uri.parse(Api.phonepeCheckSumUrl),
              body: parameter, headers: Api.getHeaders())
          .timeout(const Duration(seconds: 50));
      var getdata = json.decode(response.body);
      bool error = getdata['error'];
      if (!error) {
        ...
        initPhonePeSdk(price,
            orderId: orderID, jsonDatas: jsonData, checksums: checksum);
      }
  ...
  void initPhonePeSdk(double price,
      {required String orderId,
      Map<String, dynamic>? jsonDatas,
      String? checksums}) async {
    PhonePePaymentSdk.init(phonePeMode!, appId!, phonePeMerId!, true)
        .then((isInitialized) {
      sendRequest(orderId, "phonepe",
          OrderID: orderId,
          status: waitingKey,
          jsonDatas: jsonDatas,
          checksums: checksums);
  ...
  void startPaymentPhonePe(double price,
      {required String orderId,
      Map<String, dynamic>? jsonDatas,
      String? checksums}) async {
    try {
      ...
      PhonePePaymentSdk.startTransaction(
              body,
              /* phonePeEndPointUrl!, checksums!,  */ Platform.isAndroid
                  ? packageName
                  : iosPackage)
          .then((response) async {
        if (kDebugMode) {
  ...
  Widget walletWithdraw() {
    return BlocConsumer<GetWithdrawRequestCubit, GetWithdrawRequestState>(
        ...
        builder: (context, state) {
          if (state is GetWithdrawRequestProgress ||
              state is GetWithdrawRequestInitial) {
            return MyOrderSimmer(length: 5, width: width, height: height);
          }
          if (state is GetWithdrawRequestFailure) {
            return SizedBox(height: height! / 1.5, child: onData());
          }
          final withdrawRequestList =
              (state as GetWithdrawRequestSuccess).withdrawRequestList;
          final hasMore = state.hasMore;
          return SizedBox(
              height: height! / 1.5,
              child: withdrawRequestList.isEmpty
                  ? onData()
                  : ListView.builder(
                      shrinkWrap: true,
                      ...
                      itemBuilder: (BuildContext context, index) {
                        return hasMore &&
                                index == (withdrawRequestList.length - 1)
                            ? Center(
                                ...
                                child: Padding(
                                  ...
                                  child: Column(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                            "${UiUtils.getTranslatedLabel(context, dateLabel)} :",
                                            textAlign: TextAlign.center,
                                            style: TextStyle(
                                                color: Theme.of(context)
                                                    .colorScheme
                                        ...
                                        Text(
                                            "${UiUtils.getTranslatedLabel(context, typeLabel)} : ",
                                            textAlign: TextAlign.start,
                                            style: TextStyle(
                                                color: Theme.of(context)
                                                    .colorScheme
                                        ...
                                        withdrawRequestList[index]
                                            ...
                                            ? const SizedBox()
                                            : Text(
                                                "${UiUtils.getTranslatedLabel(context, messageLabel)} :",
                                                textAlign: TextAlign.start,
                                                style: TextStyle(
                                                    color: Theme.of(context)
                                                        .colorScheme
                                        ...
                                        Row(children: [
                                          Text(
                                              "${UiUtils.getTranslatedLabel(context, amountLabel)} : ",
                                              textAlign: TextAlign.start,
                                              style: TextStyle(
                                                  color: Theme.of(context)
                                                      .colorScheme
  ...
  Widget onData() {
    return NoDataContainer(
        ...
        subTitle:
            UiUtils.getTranslatedLabel(context, noWalletFoundSubTitleLabel),
        width: width!,
        height: height!);
  }
  ...
  Widget wallet() {
    return BlocConsumer<TransactionCubit, TransactionState>(
        ...
        builder: (context, state) {
          ...
          if (state is TransactionFailure) {
            print(
                "state:${state}--${state.errorStatusCode}--${state.errorMessage}");
            return SizedBox(height: height! / 1.5, child: onData());
          }
            ...
            children: <Widget>[
                        ...
                        Container(
                            ...
                            child: TextFormField(
                              ...
                              decoration: InputDecoration(
                                  border: InputBorder.none,
                                  hintText: UiUtils.getTranslatedLabel(
                                      context, enterWithdrawAmountLabel),
                                  labelStyle: const TextStyle(
                                    color: lightFont,
                                    fontSize: 14.0,
                                  ),
                        ...
                        Container(
                            ...
                            child: TextFormField(
                              ...
                              decoration: InputDecoration(
                                border: InputBorder.none,
                                hintText: UiUtils.getTranslatedLabel(
                                    context, enterPaymentAddressLabel),
                                labelStyle: const TextStyle(
                                  color: lightFont,
                                  fontSize: 14.0,
                                ),
              ...
              Row(
                ...
                children: [
                  ...
                  BlocConsumer<SendWithdrawRequestCubit,
                          SendWithdrawRequestState>(
                      ...
                      listener: (context, state) {
                        if (state is SendWithdrawRequestFetchSuccess) {
                          ...
                          UiUtils.setSnackBar(
                              UiUtils.getTranslatedLabel(context, paymentLabel),
                              "Withdrawal Request Sent Successfully. Wait for admin to accept the withdrawal request.",
                              context,
                              false,
                              type: "2");
                        }
                      ...
                      builder: (context, state) {
                        ...
                        if (state is SendWithdrawRequestFetchFailure) {
                          ...
                          return SmallButtonContainer(
                            ...
                            width: width,
                            text:
                                UiUtils.getTranslatedLabel(context, sendLabel),
                            start: 0,
                            end: 0,
                            bottom: height! / 60.0,
                            top: height! / 99.0,
                            ...
                            onTap: () {
                              final form = _formkey.currentState!;
                              if (form.validate() &&
                                  withdrawAmountController!.text != '0') {
                                form.save();
                                context
                                    .read<SendWithdrawRequestCubit>()
                                    .sendWithdrawRequest(
                                        context.read<AuthCubit>().getId(),
              ...
              body: BlocListener<SystemConfigCubit, SystemConfigState>(
                ...
                listener: (context, state) {
                  ...
                  if (state is SystemConfigFetchSuccess) {
                    print(
                        "state:${state.systemConfigModel.data!.userData![0].balance}");
                    walletAmount =
                        state.systemConfigModel.data!.userData![0].balance;
                  }
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      children: [
                                            ...
                                            BlocBuilder<SystemConfigCubit,
                                                ...
                                                builder: (context, state) {
                                                  if (state
                                                    ...
                                                    return Text(
                                                      "${context.read<SystemConfigCubit>().getCurrency()}${double.parse(walletAmount ?? "0.0").toStringAsFixed(2)}",
                                                      style: const TextStyle(
                                                          color: Colors.white,
                                                          fontWeight:
                                                              FontWeight.w600,
                        ...
                        Container(
                            margin:
                                EdgeInsetsDirectional.only(top: height! / 60.0),
